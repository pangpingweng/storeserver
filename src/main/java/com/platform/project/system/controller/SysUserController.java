...
//    @PreAuthorize("@ss.hasPermi('system:user:list')")
    ...
    @GetMapping("/newlist")
    public TableDataInfo newlist(SysUser user)
    {
        startPage();
        List<SysUser> list = userService.selectNewUserList(user);
        //list = list.stream().filter(s->s.getDeptId() != 100L).collect(Collectors.toList());
        return getDataTable(list);
    }


    @GetMapping("/oneList")
    public AjaxResult oneList(SysUser user)
    {
        List<SysUser> list = userService.selectUserList(user);
    ...
    @GetMapping("/twoList")
    public AjaxResult twoList(SysUser user)
    {
        startPage();
        List<SysUser> list = userService.selectUserList(user);
        return  AjaxResult.success(list);
    ...
    public AjaxResult threeList(SysUser user)
    {
        List<SysUser> list = userService.selectUserList(user);
        list = list.stream().filter(s-> (s.getDeptId()!=null && s.getDeptId().longValue() == 5L)).collect(Collectors.toList());
        return  AjaxResult.success(list);
    }
...
//        userService.checkUserDataScope(userId);
        ...
        if (StringUtils.isNotNull(userId))
        {
            ...
            if(roleList!=null && roleList.size()>0){
                sysUser.setRoleId(roleList.get(0));
            }
